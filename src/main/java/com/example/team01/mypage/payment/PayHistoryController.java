...
public class PayHistoryController {
    ...
    , HttpServletRequest request) {
        ...
        List<PaymentListDTO> paymentList = selectPaymentList(clientId);
    ...
    public ResponseEntity<?> postPayCancel(@RequestBody PaymentListVO params,
        ...
        //해당유저의 결제목록,배송지 주소 조회해오기
        List<PaymentListDTO> paymentList = selectPaymentList(clientId); // 클라이언트의 모든 결제내역 조회 쿼리
        ...
        if(canceledAll){ // 전체취소이면
            ...
            if(result > 0){
                //해당유저의 결제목록,배송지 주소 조회해오기
                paymentList = selectPaymentList(clientId);
