...
public interface PaymentDao {
    ...
    int insertPaymentList(@Param("payId") String payId, @Param("bookId") String bookId, @Param("quantity") int quantity);
    ...
    //maypage 한 PayId에 해당하는 전첼취소 결제내역 데이터 조회
     List<PaymentListVO> selectAllCancelPaymentList(@Param("payId") String payId, @Param("bookIds") List<String> bookIds, String clientId);
    
    //mypage payment 결제취소 상태 갱신(Update)
     int partialCancel(@Param("payId") String payId,@Param("clientId") String clientId,@Param("bookIds")List<String> bookIds);
     int allCancel(@Param("payId") String payId,@Param("clientId") String clientId);

    //결제취소 계산 이후 관련 데이터 조회-
     PaymentListVO  selectCancelBooksInfo (@Param("payId") String payId,@Param("bookId") String bookId);
