...
const cartAllPrice = ({cartList,selectItem,deliveryFee }) =>{
    ...
    const books= cartList?.filter(item => selectItem.includes(item.cartId)) ;
    console.log("cartAllPrice--------items",books);

    let bookPrice= 0;
    for(let i = 0; i < books?.length; i++){
        bookPrice += (books[i].bookVO.bookPrice * books[i].quantity);
        console.log("bookPrice---1",bookPrice);
    }
            ...
            <ul className="cart-item-count ul bullet border-top border-bottom py-3 mt-5 d-flex">
                ...
                <li className="d-inline-flex align-items-center ms-auto">
                    <button type="submit" aria-label="구매하기"
                            className="submit btn btn-secondary">구매하기
                    </button>
                </li>
            </ul>
        </>
