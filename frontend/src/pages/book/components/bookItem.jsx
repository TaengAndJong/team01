...
const BookItem = ({ bookList, wishIds, setWishIds }) => {
  ...
  const saleStatus = (status) => {
    switch (status) {
      case "판매중":
        return <span className="tultip d-inline-flex">{status}</span>;
      case "미판매":
        return <span className="tultip d-inline-flex popular">{status}</span>;
      case "단종":
        return <span className="tultip d-inline-flex complete">{status}</span>;
      default:
        return "판매상태 알수없음";
    }
  };
      ...
      {bookList && (
        <ul className="book-item-list clearfix d-flex flex-wrap">
          {bookList?.map((book) => (
            <li
              key={book.bookId}
              className="book-item mb-3 mx-2 overflow-hidden justify-content-between"
            >
              <Link
                to={`/book/bookDetail/${book.bookId}`}
                className="book-link d-block"
                id={book.bookId}
                onClick={(e) => viewBookFetch(e, book.bookId)}
              >
                <div className="item-inner d-flex card flex-row  position-relative">
                  ...
                  <div className="bookInfo card-body">
                    ...
                    <ul className="ul bullet">
                      <li className="li">
                        <span className="tit fw-bold">저자</span>
                        {book.author}
                      </li>
                      <li className="li">
                        <span className="tit fw-bold">발행일</span>
                        {book.publishDate}
                      </li>
                      {/*<li className="li"><span className="tit">출판사</span>{book.publishDate}</li>*/}
                      <li className="li">
                        <span className="tit fw-bold">가격</span>
                        <span>{book.bookPrice}</span>원
                      </li>
                      <li className="li">
                        <span className="tit fw-bold">배송비</span>
                        <span>2,000</span>원
                      </li>
                    </ul>
                  </div>
              ...
              <div className="item-inner d-flex align-items-center justify-content-end mt-4">
                ...
                {book.stock === 0 ? (
                  ...
                  <>
                    ...
                    <AddCartBtn
                      bookId={book.bookId}
                      quantity={bookCount[book.bookId] ?? 1}
                    />
                    {/*바로구매*/}
                    <BuySelectedBtn
                      type={"buyNow"}
                      book={{ ...book, quantity: bookCount[book.bookId] ?? 1 }}
                    />
                  </>
                )}
              </div>
