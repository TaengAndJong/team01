...
const infoForm = ({formData,onInputChange}) =>{
    ...
    const phoneChangeHandle = (e) => {
        ...
        onInputChange("phone", newPhone);
    }

    // identiNum handle
    const f_IdentiNumChangeHandle = (e) => {
        const newFidentiNum = e.target.value;
        onInputChange("f_IdentiNum", newFidentiNum);
    }

    const s_IdentiNumChangeHandle = (e) => {
        const newSidentiNum = e.target.value;
        onInputChange("s_IdentiNum", newSidentiNum);
    }
    ...
    const adrHandle = (e) => { // 사용자가 직접 입력 할 시 동작하는 handler
